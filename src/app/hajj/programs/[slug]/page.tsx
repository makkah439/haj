...
export async function generateStaticParams() {
  const programs = await getProgramsByType("hajj");
...
export async function generateMetadata({
  ...
  const program = await getProgram("hajj", params.slug);
  if (!program) {
    return {};
  }
...
export default async function HajjProgramDetailPage({
  params,
}: {
  params: { slug: string };
}) {
          ...
          <Reveal className="mx-auto max-w-2xl">
            <div className="rounded-[1.75rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-8 shadow-sm sm:p-10">
              ...
              <div className="mt-6 rounded-[1.25rem] border border-[color:var(--color-border)] p-5 text-sm leading-7 text-[color:var(--color-muted)]">
                الأسعار والمواعيد وتفاصيل الإقامة الدقيقة تختلف من موسم لآخر.
