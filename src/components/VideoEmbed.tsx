 ...
 */
export default function VideoEmbed({ url, mode, className }: { url: string; mode: 'thumbnail' | 'player'; className?: string }) {
  ...
  if (mode === 'thumbnail') {
    ...
    if (info.kind === 'vimeo' || info.kind === 'external-link') {
      ...
      return (
        <div className={`flex h-full w-full flex-col items-center justify-center gap-1 bg-[color:var(--color-primary-dark)] text-white/80 ${className ?? ''}`}>
    ...
    return <video src={url} className={`h-full w-full object-cover ${className ?? ''}`} muted preload="metadata" />;
  }
  ...
  if (info.kind === 'external-link') {
    return (
      ...
      >
        ...
        <span className="text-sm font-semibold">مشاهدة على {info.platform}</span>
      </a>
    );
  }

  return <video src={url} controls autoPlay className={`h-full w-full object-contain ${className ?? ''}`} />;
}
